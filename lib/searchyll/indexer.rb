...
module Searchyll
  class Indexer
    ...
    def es_bulk_insert!(http, batch)
      ...
      # puts(batch.map do |doc| [ { :index => {} }.to_json, doc.to_json ].join("\n") end.join("\n") + "\n"+ "\n")
      # puts bulk_insert.body

      res = http.request(bulk_insert)
      # puts res.body, JSON.parse(res.body).key?("errors")
      if JSON.parse(res.body).key?("errors")
        # if JSON.parse(res.body)["errors"] == true
        #   puts res.body 
        # end
        # puts JSON.parse(res.body).errors, JSON.parse(res.body).errors.class
        puts res.body  
      end
